...
3. ☐ **`#/politikalar`** açıldı ve `yazma` riskli araçların hepsi `Onay ister`: `jira.createIssue`, `jira.updateIssue`, `jira.addComment`, `slack.postMessage`, `github.addComment`, `gmail.createDraft`, `calendar.createEvent`, `sheets.appendRow`, `notion.createPage`. Okuma araçları `Otomatik`. Ekranın üstündeki "N araç varsayılanından farklı çalışıyor" uyarısı **görünmüyor**. (ALTIN KURAL — §4.)
