...
> **Asla canlı yazma adımını onaysız gösterme.** Demo öncesi `#/politikalar` ekranını aç ve **yedi yazma aracının yedisinde de** `Onay ister` yazdığını gör: `jira.createIssue`, `jira.updateIssue`, `jira.addComment`, `slack.postMessage`, `github.addComment`, `gmail.createDraft`, `calendar.createEvent`. Biri `Otomatik`e alınmışsa demoya başlama, önce düzelt — ekran bunu "N araç varsayılanından farklı çalışıyor" satırıyla zaten söylüyor. Onay kapısı ürünün tezi; kapısız bir yazma adımı tezle çelişir ve tek karede tüm hikâyeyi çökertir.
...
3. ☐ **`#/politikalar`** açıldı ve yedi yazma aracının yedisi de `Onay ister`: `jira.createIssue`, `jira.updateIssue`, `jira.addComment`, `slack.postMessage`, `github.addComment`, `gmail.createDraft`, `calendar.createEvent`. Okuma araçları `Otomatik`. Ekranın üstündeki "N araç varsayılanından farklı çalışıyor" uyarısı **görünmüyor**. (ALTIN KURAL — §4.)
